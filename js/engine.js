...
var Engine = (function(global) {
    ...
    var doc = global.document,
        ...
        reBtn = doc.createElement('button'),
    ...
    reBtn.style.width = "70px";
    reBtn.style.height = "30px";

    var btnText = document.createTextNode("Restart");
    reBtn.appendChild(btnText);
    doc.body.appendChild(reBtn);
    ...
    /* Called by update(dt)
     * Check the position of the player and gems;
     * Update score and let collected gem disappears.
     */
    ...
    /* Called by update(dt)
     * Check the position of the player and enemies;
     * Update score and lifeCount;
     */
    function checkCollisions(){

      var isColllided = false;

      if(player.y === 60){
        if(Math.abs(enemy1.x - player.x) < 50){
          isColllided = true;
        }
      }
      else if(player.y === 140){
        if(Math.abs(enemy2.x - player.x) < 50){
          isColllided = true;
        }
      }
      else if(player.y === 220){
        if(Math.abs(enemy3.x - player.x) < 50 || Math.abs(enemy4.x - player.x) < 50){
          isColllided = true;
        }
      }
    ...
    function renderText() {
      ...
      if(player.start === false){
        ...
        ctx.fillStyle = "White";
        ctx.fillRect(110, 600, 400, 60);
        ctx.fillRect(160, 650, 400, 70);

        ctx.font = "36px arial";
        ctx.fillStyle = "Black";
        ctx.fillText("Score:", 0, 640);
        ctx.fillText("Triumph:", 0, 690);

        ctx.font = "15px arial";
        ctx.fillText("Press Arrow key to browse player", 200, 620);
        ctx.fillText("Press Enter key to confirm", 200, 640);
      }
      else{
        ctx.fillStyle = "White";
        ctx.fillRect(110, 600, 400, 60);
        ctx.fillRect(160, 650, 100, 70);

        ctx.font = "36px arial";
        ctx.fillStyle = "Red";
        ctx.fillText(player.score, 120, 640);
        ctx.fillText(player.triumphCount, 160, 690);
      }
